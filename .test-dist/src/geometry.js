const EPSILON = 0.001;
export function rotatedSize(definition, rotation) {
    return rotation === 90 || rotation === 270
        ? { width: definition.height, height: definition.width }
        : { width: definition.width, height: definition.height };
}
export function shapePoints(shape, width, height) {
    switch (shape) {
        case 'cross':
            return [
...
export function rotatePoint(point, width, height, rotation) {
    switch (rotation) {
        case 90:
...
export function pointsFor(definition, rotation) {
...
export function rectangleGaps(a, b) {
    return {
 ...
 */
export function roomsMeetConnectionOrSpacingRule(first, second, definitions, minimumGap = 2) {
    ...
    if (firstDefinition.category !== 'room' || secondDefinition.category !== 'room')
        return true;
    ...
    if (rectanglesOverlap(firstBounds, secondBounds))
        return false;
    if (findDoorwayConnections([first, second], definitions).length > 0)
        return true;
...
export function sharedWallLength(first, second) {
    ...
    if (approximatelyEqual(firstRight, second.x) || approximatelyEqual(secondRight, first.x)) {
        sharedLength = Math.max(sharedLength, Math.min(firstBottom, secondBottom) - Math.max(first.y, second.y));
    }
    if (approximatelyEqual(firstBottom, second.y) || approximatelyEqual(secondBottom, first.y)) {
        sharedLength = Math.max(sharedLength, Math.min(firstRight, secondRight) - Math.max(first.x, second.x));
    }
...
export function roomMeetsGlobalConnectionOrSpacingRule(candidate, placed, definitions, minimumGap = 2) {
    ...
    if (otherRooms.length === 0)
        return true;
    const candidateConnections = findDoorwayConnections([candidate, ...otherRooms], definitions).filter((connection) => (connection.first.instanceId === candidate.instanceId
        || connection.second.instanceId === candidate.instanceId));
    const connectedRoomIds = new Set(candidateConnections.map((connection) => (connection.first.instanceId === candidate.instanceId
    ...
    return otherRooms.every((other) => {
        ...
        if (rectanglesOverlap(candidateBounds, otherBounds))
            return false;
        ...
        if (cornerOnlyContact)
            return candidateHasConnection;
        return gaps.x >= minimumGap || gaps.y >= minimumGap;
    });
}
...
export function transformDoorway(doorway, structureWidth, structureHeight, rotation) {
...
export function doorwayKey(instanceId, doorwayIndex) {
    return `${instanceId}:${doorwayIndex}`;
}
export function worldDoorways(item, definition) {
...
export function findDoorwayConnections(placed, definitions) {
    ...
    for (let firstIndex = 0; firstIndex < doorways.length; firstIndex += 1) {
        ...
        for (let secondIndex = firstIndex + 1; secondIndex < doorways.length; secondIndex += 1) {
            ...
            if (first.instanceId === second.instanceId)
                continue;
            if (oppositeSide(first.side) !== second.side)
                continue;
            if (!approximatelyEqual(first.line, second.line))
                continue;
            if (!approximatelyEqual(first.spanStart, second.spanStart))
                continue;
            if (!approximatelyEqual(first.spanEnd, second.spanEnd))
                continue;
...
function spansOverlap(firstStart, firstEnd, secondStart, secondEnd) {
 ...
 */
export function findBlockedDoorwayKeys(placed, definitions, connectedKeys) {
    ...
    for (const item of placed) {
        ...
        for (const doorway of worldDoorways(item, definition)) {
            if (connectedKeys.has(doorway.key))
                continue;
            ...
            if (hitsWall)
                blocked.add(doorway.key);
        }
    }
    return blocked;
}
export function doorwayRect(item, doorway, thickness = 0.55) {
