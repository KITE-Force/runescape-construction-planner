import { findNearestValidSelectionPlacement, rotateSelectionClockwise, rotateSelectionCounterClockwise, selectionBounds, translateSelection, } from '../src/groupTransform.js';
...
assert(roundTrip.every((item, index) => (item.x === group[index].x
    && item.y === group[index].y
    && item.rotation === group[index].rotation)), 'counter-clockwise followed by clockwise rotation should restore the selection');
const reportedLargeSquare = {
...
const reportedCornerBeforeRotation = {
...
const reportedRotatedInPlace = rotateSelectionClockwise([reportedCornerBeforeRotation], structureById);
assert(reportedRotatedInPlace[0].x === 7
    && reportedRotatedInPlace[0].y === 10
    && reportedRotatedInPlace[0].rotation === 180, 'the basic rotation should first try the same stored origin');
const reportedPlacementIsValid = (items) => {
    ...
    return !rectanglesOverlap(candidateBounds, squareBounds)
        && roomMeetsGlobalConnectionOrSpacingRule(candidate, [reportedLargeSquare], structureById);
};
assert(!reportedPlacementIsValid(reportedRotatedInPlace), 'rotating the reported corner while pinning its top-left origin should be invalid');
const smartRotatedPlacement = findNearestValidSelectionPlacement(reportedRotatedInPlace, reportedPlacementIsValid, 4);
assert(smartRotatedPlacement !== null, 'smart rotation should find a nearby legal placement');
assert(smartRotatedPlacement.dx === 0
    && smartRotatedPlacement.dy === 2
    && smartRotatedPlacement.items[0].x === 7
    && smartRotatedPlacement.items[0].y === 12
    && smartRotatedPlacement.items[0].rotation === 180, 'smart rotation should reproduce the valid exported corner placement by nudging it two tiles down');
