...
Select one or more structures and use the **Structure color** controls in the Selection editor. The browser color picker previews changes immediately without filling the recent-color history with intermediate values. The text field accepts `#RRGGBB`, three-digit hex, `rgb(r, g, b)`, or comma-separated RGB values. **Apply** commits the final color to the full selection and Recently used; **Reset** restores each selected item to its default category color. Custom colors are stored in local saves and JSON imports/exports.

## Recently used colors

Committed structure colors are saved locally in the browser and shown as one-click swatches in the Selection color controls. Dragging through the native color picker only previews colors; intermediate picker values are not recorded. The list keeps the eight most recent unique colors and can be cleared without changing any placed structures.
